...
"""Poker specific rethink sampler."""
...
def _sample_parse_return(
  ...
  # Redefined to avoid protected access alerts.
...
class PokerRethinkStrategy(enum.Enum):
  RETHINK = "rethink"
  ...
  RETHINK_REPEATED_POKER = "rethink_repeated_poker"


class RethinkSampler(rethink.RethinkSampler[PokerRethinkStrategy]):
  """Poker specific rethink sampler."""

  def _is_rethink_strategy(self, strategy: PokerRethinkStrategy) -> bool:
    return strategy in (
        PokerRethinkStrategy.RETHINK,
        PokerRethinkStrategy.RETHINK_WITH_ENV,
        PokerRethinkStrategy.RETHINK_WITH_ENV_ILLEGAL_HISTORY,
        PokerRethinkStrategy.RETHINK_WITH_ENV_RULE,
        PokerRethinkStrategy.RETHINK_REPEATED_POKER,
    )

  def is_rethink_template(self, prompt_template: str | None) -> bool:
    """Separate method to allow for per-game overrides."""
    return prompt_template in prompts.RETHINK_PROMPTS.union(
        {poker_prompt_templates.REPEATED_POKER}
    )
  ...
  def _sample_action(
    ...
    # TODO(Sohier Dane): refactor so the common code isn't duplicated.
    ...
    while (
      ...
      legal_action_strings = game_adapter.legal_actions
      ...
      # TODO(John Schultz): Remove this temporary fix for Poker.
      if (
          maybe_legal_action not in legal_action_strings
          and num_attempts == self._num_max_rethinks
      ):
        auxiliary_outputs["illegal_action_remapped"] = maybe_legal_action
        default_action = legal_action_strings[0]
        _TELEMETRY(
            illegal_action={
        ...
        maybe_legal_action = default_action
      ...
      match self._strategy:
        ...
        case PokerRethinkStrategy.RETHINK_WITH_ENV_ILLEGAL_HISTORY:
          if parsed_action is None:
            rethink_prompt = (
                prompt_templates.RETHINK_WITH_ENV_UNPARSABLE.format(
                    generation=generate_returns[-1].main_response
        ...
        case PokerRethinkStrategy.RETHINK_WITH_ENV_RULE:
          if parsed_action is None:
            rethink_prompt = (
                prompt_templates.RETHINK_WITH_ENV_UNPARSABLE.format(
                    generation=generate_returns[-1].main_response
        ...
        case PokerRethinkStrategy.RETHINK_REPEATED_POKER:
          generation = generate_returns[-1].main_response
          if not generation:
            generation = "NO RESPONSE RECEIVED"
          else:
            generation = "\n".join(generation.split("\n")[-5:])
          rethink_prompt = poker_prompt_templates.POKER_RETHINK.format(
    ...
    return samplers.SamplerOutput(
        ...
        if maybe_legal_action is not None
        else samplers.MoveType.ILLEGAL,
    )
