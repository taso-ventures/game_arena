...
[Game]
universal_poker(betting=nolimit,bettingAbstraction=fullgame,blind=100 50,firstPlayer=2 1 1 1,numBoardCards=0 3 1 1,numHoleCards=2,numPlayers=2,numRanks=13,numRounds=4,numSuits=4,stack=20000 20000)
[State]
3
39
15
...
"""
    ...
    readable_state = game_adapter.get_readable_state()
    self.assertEqual(
        readable_state,
...
Action Sequence: ddddccdddpcdccd""",
  ...
  def test_get_readable_state_poker_player_1(self):
    observation = dict()
    observation[
...
"""
    ...
    game_adapter = poker_game.PokerGameAdapter()
    game_adapter.update_game_state(observation)
    readable_state = game_adapter.get_readable_state()
    self.assertEqual(
        readable_state,
        """BettingAbstraction: FULLGAME
...
class TestFormatChessMovetext(absltest.TestCase):
  ...
  def test_format_chess_movetext_default(self):
    self.assertEqual(
        chess_game.format_chess_movetext(
            self.game,
...
class TestGetPgn(absltest.TestCase):
  ...
  def test_get_pgn_with_custom_player_names(self):
    game = pyspiel.load_game('chess')
    state = game.new_initial_state()
    pgn = chess_game.get_pgn(state, player_names=['Player A', 'Player B'])
...
1. e4 g5 2. Nc3 f5 3. Qh5# 1-0"""
    ...
    pgn_from_state = chess_game.get_pgn(spiel_state)
...
class TestGetActionStringHistory(absltest.TestCase):
  ...
  def test_get_action_string_history_chess_initial_state(self):
    ...
    game_adapter.native_state = game.new_initial_state()
    self.assertEqual(game_adapter.get_action_string_history(), '1.')

  def test_get_action_string_history_chess_with_moves(self):
    game = pyspiel.load_game('chess')
    game_adapter = chess_game.ChessGameAdapter()
    game_adapter.native_state = game.new_initial_state()
    game_adapter.native_state.apply_action(
        game_adapter.native_state.string_to_action('e4')
    )
    game_adapter.native_state.apply_action(
        game_adapter.native_state.string_to_action('e5')
    )

    self.assertEqual(game_adapter.get_action_string_history(), '1. e4 e5 2.')
...
class TestGetChessPiecePositions(absltest.TestCase):
  ...
  def test_sicilian_defense_position(self):
    ...
    result = chess_game.get_chess_piece_positions(fen)
  ...
  def test_empty_squares(self):
    ...
    result = chess_game.get_chess_piece_positions(
  ...
  def test_empty_board(self):
    ...
    expected = {}
    self.assertDictEqual(chess_game.get_chess_piece_positions(fen), expected)
...
a b c d e f g h"""
    ...
    self.assertEqual(
        chess_game.get_chess_ascii_board(
            pyspiel_state.board().to_fen(), caption=True
        ),
        expected_ascii,
    )
