...
class FreeCivAction(BaseModel):
  ...
  strategic_score: float = Field(
      ...
      le=1.0,
      description=(
          "Strategic importance score for action prioritization (0.0-1.0)"
      ),
  )
  ...
  def validate_action_type(cls, v):
    ...
    if not any(v.startswith(prefix) for prefix in valid_prefixes):
      raise ValueError(
          f"Invalid action type: {v}. Must start with {valid_prefixes}"
      )
    return v
...
class FreeCivCity(BaseModel):
  ...
  population: int = Field(
      ..., ge=1, le=50
  )  # Cities typically max at around 30-40
  ...
  trade_routes: List[int] = Field(
      default_factory=list
  )  # IDs of connected cities
...
class FreeCivState(_GameStateBase):
  ...
  def __init__(self, raw_state: Mapping[str, Any]):
    ...
    self._observation_cache: LRUCache = LRUCache(
        max_size=OBSERVATION_CACHE_SIZE
    )
  ...
  def _parse_scores(self, scores_data: Mapping[str, Any]) -> Dict[str, int]:
    ...
    for k, v in scores_data.items():
      player_id = _safe_int_conversion(
          k, MAX_PLAYER_ID, f"score player_id '{k}'"
      )
      score_value = _safe_int_conversion(
          v, sys.maxsize, f"score for player {k}"
      )
  ...
  def get_prioritized_legal_actions(
    ...
    selected_actions = self._ensure_action_diversity(
        scored_actions, max_actions
    )
  ...
  def _ensure_action_diversity(
    ...
    max_per_type = max(
        max_actions // 4, 2
    )  # At most 1/4 of actions of same type
  ...
  def apply_action(self, action: FreeCivAction) -> None:
    if action.source == "unit":
      ...
      self._invalidate_caches(
          "cities"
      )  # City actions affect cities and actions
    ...
    for city in cities:
      if city["owner"] == player_id:
        ...
        compressed.append({
            ...
            "name": city["name"],
            "pop": city["population"],
            "production": city.get("production", {}).get("current", "unknown"),
            "x": city["x"],
            "y": city["y"],
        })
      else:
