...
from game_arena.harness import (model_generation, parsers, prompt_generation,
                                prompts, rethink, tournament_util)
...
class _DummyPromptGeneratorSupportsText(prompt_generation.PromptGeneratorSupportsText):
...
class RethinkSamplerTest(absltest.TestCase):
    ...
    def test_sample_action_text_only_success_first_try(
        ...
        mock_legality_parser = mock.create_autospec(parsers.TextParser, instance=True)
    ...
    def test_sample_action_text_only_fail_after_max_rethinks(
        ...
        self.assertEqual(output.generate_returns, [illegal_return1, illegal_return2])
        self.assertDictEqual(
            output.auxiliary_outputs,
            {
    ...
    def test_rethink_with_env_strategy_unparsable(self, mock_get_legal_action_strings):
    ...
    def test_image_based_workflow(
        ...
        mock_generate_fn.return_value = generate_return
        mock_prompt_generator.generate_prompt_with_image_text.return_value = "prompt"
        mock_move_parser.parse.return_value = "e4"
        mock_legality_parser.parse.side_effect = _fake_legality_parser_parse
