...
class ModelGenerationHttpTest(parameterized.TestCase):
    ...
    def test_deepseek_separate_main_response_and_thoughts(self, response, expected):
        self.assertEqual(
...
class XAIModelTest(absltest.TestCase):
    ...
    def test_generate_with_reasoning(self, mock_post):
        ...
        model = model_generation_http.XAIModel(model_name="grok", api_key="fake_key")
    ...
    def test_generate_streaming(self, mock_post):
        ...
        self.assertIsInstance(
            response.response_for_logging["filtered_chunks_list"][0]["elapsed_time"],
            float,
        )
...
class PostRequestsAsyncReturnFirstSuccessTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cancellation_does_not_wait_for_long_task(self, mock_post_async):
        # Arrange
        async def side_effect(*, name, **kwargs):
            del kwargs  # Unused
            if name == "fast":
