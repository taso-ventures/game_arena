...
class FreeCivProxyClient:
    ...
    async def get_state(self, format_type: str = "llm_optimized") -> Dict[str, Any]:
        ...
        # Check cache first
        cache_key = f"state_{format_type}"
        ...
        if (
            ...
            return {
                k: v
                for k, v in self.state_cache[cache_key].items()
                if k != "_timestamp"
            }
        ...
        if response:
            ...
            if len(self.state_cache) >= self.max_cache_entries:
                # Remove oldest entry
                oldest_key = min(
                    self.state_cache.keys(),
                    key=lambda k: self.state_cache[k].get("_timestamp", 0),
                )
        ...
        if tasks:
            try:
                ...
                for task in pending:
                    task.cancel()
                    logger.warning(
                        f"Force cancelled background task: {task.get_name()}"
                    )
    ...
    async def get_next_message(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            _, _, message = await asyncio.wait_for(
                self.priority_queue.get(), timeout=0.001
            )
            return message
        except asyncio.TimeoutError:
            pass
        ...
        try:
            message = await asyncio.wait_for(self.normal_queue.get(), timeout=0.001)
            return message
        except asyncio.TimeoutError:
            return None
