...
from game_arena.harness import (model_generation, model_generation_sdk,
                                tournament_util)
...
class AIStudioModelTest(absltest.TestCase):
    ...
    def test_generate_with_text_input_with_thoughts(self, mock_client_constructor):
        mock_generative_model = mock.Mock()
        mock_client = mock_client_constructor.return_value
        mock_client.models = mock_generative_model
    ...
    def test_generate_with_image_text_input(self, mock_client_constructor):
        ...
        contents = [
            "prompt text",
            google_genai_types.Part.from_bytes(data=image_bytes, mime_type="image/png"),
        ]
        self.assertEqual(
            response,
            tournament_util.GenerateReturn(
...
class OpenAIChatCompletionsModelTest(absltest.TestCase):
    ...
    @mock.patch("game_arena.harness.model_generation_sdk.openai.OpenAI", spec=True)
    def test_generate_with_text_input_streaming(self, mock_client_constructor):
        ...
        self.assertEqual(
            ...
            tournament_util.GenerateReturn(
                ...
                response_for_logging={"response_chunks": [c.to_dict() for c in stream]},
            ),
        )
    ...
    @mock.patch("game_arena.harness.model_generation_sdk.openai.OpenAI", spec=True)
    def test_generate_with_text_input_non_streaming(self, mock_client_constructor):
        mock_chat = mock.Mock()
        mock_completions = mock.Mock()
        mock_chat.completions = mock_completions
        mock_client = mock_client_constructor.return_value
    ...
    def test_process_anthropic_stream(self):
        mock_stream = [
            ...
            anthropic_types.RawContentBlockDeltaEvent(
                delta=anthropic_types.TextDelta(text="response ", type="text_delta"),
