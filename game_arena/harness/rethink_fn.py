...
def rule_explain_illegal_move(*, fen: str, move_str: str):
        ...
        try:
            ...
            if "ambiguous" in error_msg:
                ...
                try:
                    ...
                    if p_match is None:
                        raise ValueError(
                            f"Invalid SAN: {move_str}"
                        ) from parse_san_error
                    else:
    ...
    if piece_type == chess.PAWN:
        ...
        if target_piece is None and chess.square_file(from_square) != chess.square_file(
            to_square
        ):
        ...
        if target_piece is not None and chess.square_file(
            ...
            return "Illegal Pawn Move: Pawns cannot capture by moving straight forward."
        return (
    ...
    if piece_type in (chess.ROOK, chess.BISHOP, chess.QUEEN):
        ...
        if piece_type == chess.ROOK:
            return "Illegal Rook Move: A rook can only move horizontally or vertically."
    ...
    return (
        "Illegal Move: The move violates the rules of chess for an unknown" " reason."
    )
