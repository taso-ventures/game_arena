...
def main(_) -> None:
  ...
  for move_number in range(_NUM_MOVES.value):
    print(
        f"\nPre-move state summary: Turn {freeciv_game_state.turn}, Phase"
        f" {freeciv_game_state.phase}"
    )
    ...
    prompt_substitutions = {
        "readable_state_str": (
            f"Turn {freeciv_game_state.turn}: {len(legal_actions)} actions"
            " available"
        ),
        "move_history": (
            f"Turn {freeciv_game_state.turn} - {freeciv_game_state.phase} phase"
        ),
    ...
    if "tactical" in observation:
      ...
      prompt_substitutions["readable_state_str"] += (
          f"\nUnits: {unit_counts.get('friendly', 0)} friendly,"
          f" {unit_counts.get('enemy', 0)} enemy"
      )
      prompt_substitutions[
          "readable_state_str"
