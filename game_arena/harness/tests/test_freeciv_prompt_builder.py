...
class TestObservationBuilder(unittest.TestCase):
  ...
  def test_identify_opportunities(self):
    ...
    opportunities = self.observation_builder._identify_opportunities(
        self.mock_obs
    )
...
class TestContextManager(unittest.TestCase):
  ...
  def test_compress_observation(self):
    ...
    self.assertLessEqual(
        len(compressed.get("units", [])), len(large_obs["units"])
    )
...
class TestEdgeCases(unittest.TestCase):
  ...
  def test_context_manager_with_empty_observations(self):
    ...
    prioritized = context_manager.prioritize_information(
        empty_obs, "early_game"
    )
