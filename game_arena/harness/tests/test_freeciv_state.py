...
class TestFreeCivState(unittest.TestCase):
  ...
  def test_state_initialization(self):
    ...
    self.assertEqual(
        state.turn, 10, "Turn should be correctly parsed from fixture"
    )
    self.assertEqual(
    ...
    self.assertEqual(
        state.map.height, 4, "Map height should match fixture data"
    )
    ...
    self.assertFalse(
        state.is_terminal(), "Early game state should not be terminal"
    )
  ...
  def test_freeciv_action_json_parsing(self):
    ...
    json_string = (
        '{"action": "unit_attack", "actor": 102, "target": {"id": 203}}'
    )
  ...
  def test_prioritized_legal_actions(self):
    ...
    prioritized = state.get_prioritized_legal_actions(
        player_id=1, max_actions=10
    )
  ...
  def test_comprehensive_performance_benchmarks(self):
    ...
    self.assertLess(
        avg_time_ms, 1, f"Packet conversion took {avg_time_ms:.2f}ms"
    )
    ...
    avg_time_ms = ((end_time - start_time) / 100) * 1000
    self.assertLess(
        avg_time_ms, 5, f"Strategic scoring took {avg_time_ms:.2f}ms"
    )
