...
class FreeCivParserConfig:
  ...
  Attributes:
    ...
    max_line_length: Maximum length for individual lines to prevent memory exhaustion
  ...
  max_line_length: int = 10 * 1024  # 10KB per line
  ...
  def from_environment(cls, **overrides) -> "FreeCivParserConfig":
    ...
    if debug := os.getenv("FREECIV_PARSER_ENABLE_DEBUG"):
      env_overrides["enable_debug_logging"] = debug.lower() in (
          "1",
          "true",
          "yes",
      )
    ...
    if perf_log := os.getenv("FREECIV_PARSER_ENABLE_PERF_LOG"):
      env_overrides["enable_performance_logging"] = perf_log.lower() in (
          "1",
          "true",
          "yes",
      )
