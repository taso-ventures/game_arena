...
class FreeCivClient:
  ...
  def connect(self) -> None:
    ...
    try:
      ...
      self.connected = True
      logger.info(
          f"Connected to FreeCiv3D server, game_id: {self.game_id}, player_id:"
          f" {self.player_id}"
      )
  ...
  async def _async_connect_websocket(self) -> None:
    ...
    try:
      ...
      logger.warning(
          f"WebSocket connection failed, continuing in HTTP mode: {e}"
      )
      self.websocket = None
  ...
  def _get_mock_game_state(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "map": {
            ...
            "visibility": {
                str(self.player_id): [
                    [i, j] for i in range(6) for j in range(4)
                ]
            },
        },
        "players": [{
  ...
  async def _async_submit_action_via_websocket(
      self, action: FreeCivAction
  ) -> bool:
